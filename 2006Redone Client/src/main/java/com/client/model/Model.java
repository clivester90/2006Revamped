package com.client.model;// Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) 

import com.*;
import com.client.model.frames.Frames;
import com.client.model.frames.FramesBase;
...
public class Model extends SceneNode {
	...
	public static void loadModel(byte[] data, int j) {
		...
		header.data = data;
		header.vertexAmount = stream.readUnsignedWord();
		header.faceAmount = stream.readUnsignedWord();
		header.texturedFaceAmount = stream.readUnsignedByte();
		int useTextures = stream.readUnsignedByte();
		int useFacePriority = stream.readUnsignedByte();
		int useTransparency = stream.readUnsignedByte();
		int useFaceSkinning = stream.readUnsignedByte();
		int useVertexSkinning = stream.readUnsignedByte();
		int xDataOffset = stream.readUnsignedWord();
		int yDataOffset = stream.readUnsignedWord();
		int zDataOffset = stream.readUnsignedWord();
		int faceDataLength = stream.readUnsignedWord();
		int offset;
	...
	private Model(int i) {
		...
		Stream stream = new Stream(modelHeader.data);
		stream.currentOffset = modelHeader.vertexDirectionOffset;
		Stream stream_1 = new Stream(modelHeader.data);
		stream_1.currentOffset = modelHeader.xDataOffset;
		Stream stream_2 = new Stream(modelHeader.data);
		stream_2.currentOffset = modelHeader.yDataOffset;
		Stream stream_3 = new Stream(modelHeader.data);
		stream_3.currentOffset = modelHeader.zDataOffset;
		Stream stream_4 = new Stream(modelHeader.data);
		stream_4.currentOffset = modelHeader.vertexSkinOffset;
		int k = 0;
		int l = 0;
		int i1 = 0;
		...
		stream.currentOffset = modelHeader.colorDataOffset;
		stream_1.currentOffset = modelHeader.texturePointerOffset;
		stream_2.currentOffset = modelHeader.facePriorityOffset;
		stream_3.currentOffset = modelHeader.faceAlphaOffset;
		stream_4.currentOffset = modelHeader.faceSkinOffset;
		...
		stream.currentOffset = modelHeader.faceDataOffset;
		stream_1.currentOffset = modelHeader.faceTypeOffset;
		int j2 = 0;
		int l2 = 0;
		int j3 = 0;
		int k3 = 0;
		...
		stream.currentOffset = modelHeader.uvMapFaceOffset;
